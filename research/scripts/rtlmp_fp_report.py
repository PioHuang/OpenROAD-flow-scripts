#!/usr/bin/env python3
"""Plot RTLMP soft-macro floorplan from HierRTLMP `root.fp.txt` (same format as mempool `report.py`)."""
...
def classify(name: str, width_dbu: int, height_dbu: int) -> str:
    ...
    if width_dbu <= 1 or height_dbu <= 1:
        return "point"
    # Mempool-style macro leaves
    if "sram_instance" in name or "fr_sp_instance" in name:
        return "hard_macro"
    # Broader heuristic (e.g. BlackParrot fakeram / wrapper hierarchies)
    if "_wrapper" in name or "wrapper" in name.lower():
        return "hard_macro"
    return "cluster"
...
def main() -> None:
    ap = argparse.ArgumentParser(description="RTLMP root.fp.txt → clusters CSV/PNG/report.")
    ap.add_argument("--fp", required=True, type=Path)
    ...
    ap.add_argument("--core-lx", type=float, required=True)
    ap.add_argument("--core-ly", type=float, required=True)
    ap.add_argument("--core-ux", type=float, required=True)
    ap.add_argument("--core-uy", type=float, required=True)
    args = ap.parse_args()

    rows = read_fp(args.fp, args.dbu_per_micron)
